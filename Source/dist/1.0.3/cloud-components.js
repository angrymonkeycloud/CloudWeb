...
(function (root, factory) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        if (typeof options.template !== 'string' && !(options.template instanceof $))
            options.template =
                ...
                '<div class="calendar-years" style="display: none;"></div>' +
                '<div class="calendar-months" style="display: none;"></div>' +
        ...
        if (!this.timePicker) {
            this.startDate = this.GetStartDate(true).startOf('day');
        ...
        this.container.find('.drp-calendar')
            .on('click.daterangepicker', '.calendar-months .year', $.proxy(this.clickYear, this))
            .on('click.daterangepicker', '.calendar-years .yearsList > div', $.proxy(this.yearClick, this))
            .on('click.daterangepicker', '.month', $.proxy(this.clickMonth, this))
            .on('click.daterangepicker', '.calendar-months .monthsList > div', $.proxy(this.monthClick, this))
            .on('click.daterangepicker', '.prev', $.proxy(this.clickPrev, this))
            .on('click.daterangepicker', '.next', $.proxy(this.clickNext, this))
            .on('mousedown.daterangepicker', 'td span.available', $.proxy(this.clickDate, this))
            .on('mouseenter.daterangepicker', 'td span.available', $.proxy(this.hoverDate, this))
    ...
    DateRangePicker.prototype = {
        ...
        setEndDate: function (endDate) {
            ...
            if (this.endDate.isBefore(this.GetStartDate(false)))
                this.endDate = this.GetStartDate(false).clone();

            if (this.maxDate && this.endDate.isAfter(this.maxDate))
                this.endDate = this.maxDate.clone();

            if (this.maxSpan && this.GetStartDate(false).clone().add(this.maxSpan).isBefore(this.endDate))
                this.endDate = this.GetStartDate(false).clone().add(this.maxSpan);

            this.previousRightTime = this.endDate.clone();

            this.container.find('.drp-Selected').html(this.GetStartDate(false).format(this.locale.format) + this.locale.separator + this.endDate.format(this.locale.format));
        ...
        updateMonthsInView: function () {
            if (this.endDate) {
                ...
                if (!this.singleDatePicker && this.leftCalendar.month && this.rightCalendar.month &&
                    (this.GetStartDate(false).format('YYYY-MM') == this.leftCalendar.month.format('YYYY-MM') || this.GetStartDate(false).format('YYYY-MM') == this.rightCalendar.month.format('YYYY-MM'))
                    &&
                ...
                this.leftCalendar.month = this.GetStartDate(false).clone().date(2);
                if (!this.linkedCalendars && (this.endDate.month() != this.GetStartDate(false).month() || this.endDate.year() != this.GetStartDate(false).year())) {
                    this.rightCalendar.month = this.endDate.clone().date(2);
                } else {
                    this.rightCalendar.month = this.GetStartDate(false).clone().date(2).add(1, 'month');
                }

            } else {
                if (this.leftCalendar.month.format('YYYY-MM') != this.GetStartDate(false).format('YYYY-MM') && this.rightCalendar.month.format('YYYY-MM') != this.GetStartDate(false).format('YYYY-MM')) {
                    this.leftCalendar.month = this.GetStartDate(false).clone().date(2);
                    this.rightCalendar.month = this.GetStartDate(false).clone().date(2).add(1, 'month');
                }
            }
        ...
        updateCalendars: function () {
            ...
            this.renderYearPicker('left');
            this.renderYearPicker('right');

            this.renderMonthPicker('left');
            this.renderMonthPicker('right');

            this.renderCalendar('left');
            this.renderCalendar('right');

            //highlight any predefined range matching the current start and end dates
            this.container.find('.ranges li').removeClass('Selected');
            if (this.endDate == null) return;

            this.calculateChosenLabel();
        },

        renderYearPicker: function (side) {

            var calendar = side == 'left' ? this.leftCalendar : this.rightCalendar;

            var minDate = side == 'left' ? this.minDate : this.GetStartDate(false);
            var maxDate = this.maxDate;
            ...
            var html = '<div class="yearsList">';

            var data = '';
            for (var y = startYear; y <= endYear; y++) {
                if (y >= minYear && y <= maxYear) {
                    data += "<div data-year='" + y + "' class='AccentBackgroundHover ForegroundTextHover AccentBorderSelected AccentTextSelected" +
                        (y === currentYear ? " Selected" : "") +
                        "'>" + y + "</div>";
                } else {
                    data += "<div data-year='" + y + "' class='AccentBackgroundHover ForegroundTextHover AccentBorderSelected AccentTextSelected" +
                        (y === currentYear ? " Selected" : "") +
                        "' disabled='disabled'>" + y + "</div>";
                }
            }
            ...
            this.container.find('.drp-calendar.' + side + ' .calendar-years').html(html);
        },

        renderMonthPicker: function (side) {
            ...
            var html = '<div class="year AccentBackgroundHover ForegroundTextHover">' + currentYear + '</div>';

            html += '<div class="monthsList">';

            var monthHtml = '';
            for (var m = 0; m < 12; m++) {
                if ((!inMinYear || m >= minDate.month()) && (!inMaxYear || m <= maxDate.month())) {
                    monthHtml += "<div data-month='" + m + "' class='AccentBackgroundHover ForegroundTextHover AccentBorderSelected AccentTextSelected" +
                        (m === currentMonth ? " Selected" : "") +
                        "'><span>" + (m + 1) + "</span>" + this.locale.monthNames[m] + "</div>";
                } else {
                    monthHtml += "<div data-month='" + m + "' class='AccentBackgroundHover ForegroundTextHover AccentBorderSelected AccentTextSelected" +
                        (m === currentMonth ? " Selected" : "") +
                        " disabled='disabled'>" + this.locale.monthNames[m] + "</div>";
                }
            }
            ...
            this.container.find('.drp-calendar.' + side + ' .calendar-months').html(html);
        },
        ...
        renderCalendar: function (side) {
            ...
            var minDate = side == 'left' ? this.minDate : this.GetStartDate(false);
            var maxDate = this.maxDate;
            var Selected = side == 'left' ? this.GetStartDate(false) : this.endDate;
            ...
            if ((!minDate || minDate.isBefore(calendar.firstDay)) && (!this.linkedCalendars || side == 'left'))
                html += '<th class="prev available AccentBackgroundHover ForegroundBorderHover"><span></span></th>';
            else
                html += '<th></th>';

            var dateHtml = this.locale.monthNames[calendar[1][1].month()] + calendar[1][1].format(" YYYY");

            html += '<th colspan="5" class="month AccentBackgroundHover ForegroundTextHover">' + dateHtml + '</th>';
            if ((!maxDate || maxDate.isAfter(calendar.lastDay)) && (!this.linkedCalendars || side == 'right' || this.singleDatePicker))
                html += '<th class="next available AccentBackgroundHover ForegroundBorderHover"><span></span></th>';
            else
                html += '<th></th>';

            html += '</tr>';
            html += '<tr class="DaysLabel">';
            ...
            if (this.endDate == null && this.maxSpan) {
                var maxLimit = this.GetStartDate(false).clone().add(this.maxSpan).endOf('day');
            ...
            for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    if (calendar[row][col].format('YYYY-MM-DD') == this.GetStartDate(false).format('YYYY-MM-DD'))
                    ...
                    if (this.endDate != null && calendar[row][col] > this.GetStartDate(false) && calendar[row][col] < this.endDate)
                    ...
                    html += '<td><span class="AccentBorderSelected ForegroundTextHover AccentBackgroundHover AccentTextSelected ' + cname.replace(/^\s+|\s+$/g, '') + '" data-title="' + 'r' + row + 'c' + col + '">' + calendar[row][col].date() + '</span></td>';
        ...
        renderTimePicker: function (side) {
            ...
            if (this.maxSpan && (!this.maxDate || this.GetStartDate(false).clone().add(this.maxSpan).isAfter(this.maxDate)))
                maxDate = this.GetStartDate(false).clone().add(this.maxSpan);

            if (side == 'left') {
                Selected = this.GetStartDate(false).clone();
                minDate = this.minDate;
            } else if (side == 'right') {
                Selected = this.endDate.clone();
                minDate = this.GetStartDate(false);
                ...
                if (Selected.isBefore(this.GetStartDate(false)))
                    Selected = this.GetStartDate(false).clone();
        ...
        updateFormInputs: function () {
            ...
            if (this.singleDatePicker || (this.endDate && (this.GetStartDate(false).isBefore(this.endDate) || this.GetStartDate(false).isSame(this.endDate)))) {
        ...
        show: function (e) {
            ...
            this.oldStartDate = this.GetStartDate(false).clone();
        ...
        hide: function (updateElement) {
            ...
            if (!this.GetStartDate(false).isSame(this.oldStartDate) || !this.endDate.isSame(this.oldEndDate))
                this.callback(this.GetStartDate(false).clone(), this.endDate.clone(), this.chosenLabel);
            ...
            this.container.find('.calendar-years').hide();
            this.container.find('.calendar-months').hide();
            this.container.find('.calendar-table').show();
        },
        ...
        outsideClick: function (e) {
            ...
            // itself then call this.hide(true)
            if (
                // ie modal dialog fix
                e.type == "focusin" ||
        ...
        clickRange: function (e) {
            ...
            if (label == this.locale.customRangeLabel) {
                ...
                if (!this.timePicker) {
                    this.GetStartDate(false).startOf('day');
        ...
        clickYear: function (e) {
            ...
            cal.find('.calendar-years').show();
            cal.find('.calendar-months').hide();
            cal.find('.calendar-table').hide();
            this.move();
        },
        ...
        yearClick: function (e) {
            ...
            var y = $(e.target).data('year');

            if (cal.hasClass('left')) {

                this.leftCalendar.month.year(y);

                if (this.linkedCalendars)
                    this.rightCalendar.month.year(y);
            ...
            cal.find('.calendar-years').hide();
            cal.find('.calendar-months').show();
            this.move();
        },
        ...
        monthClick: function (e) {
            ...
            var m = $(e.target).data('month');

            if (cal.hasClass('left')) {

                this.leftCalendar.month.month(m);

                if (this.linkedCalendars)
                    this.rightCalendar.month.month(m);
        ...
        hoverDate: function (e) {
            ...
            var startDate = this.GetStartDate(false);
            if (!this.endDate) {
        ...
        clickDate: function (e) {
            ...
            if (this.endDate || date.isBefore(this.GetStartDate(false), 'day')) { //picking start
                if (this.timePicker) {
            ...
            } else if (!this.endDate && date.isBefore(this.GetStartDate(false))) {
                //special case: clicking the same date for start/end,
                //but the time of the end date is before the start date
                this.setEndDate(this.GetStartDate(false).clone());
            } else { // picking end
                if (this.timePicker) {
            ...
            if (this.singleDatePicker) {
                this.setEndDate(this.GetStartDate(false));
                if (!this.timePicker)
                    this.clickApply();
            }
        ...
        calculateChosenLabel: function () {
            ...
            for (var range in this.ranges) {
                if (this.timePicker) {
                    ...
                    if (this.GetStartDate(false).format(format) == this.ranges[range][0].format(format) && this.endDate.format(format) == this.ranges[range][1].format(format)) {
                        customRange = false;
                        this.chosenLabel = this.container.find('.ranges li:eq(' + i + ')').addClass('Selected').attr('data-range-key');
                        break;
                    }
                } else {
                    //ignore times when comparing dates if time picker is not enabled
                    if (this.GetStartDate(false).format('YYYY-MM-DD') == this.ranges[range][0].format('YYYY-MM-DD') && this.endDate.format('YYYY-MM-DD') == this.ranges[range][1].format('YYYY-MM-DD')) {
                        customRange = false;
                        this.chosenLabel = this.container.find('.ranges li:eq(' + i + ')').addClass('Selected').attr('data-range-key');
                        break;
                    }
            ...
            if (customRange) {
                if (this.showCustomRangeLabel) {
                    this.chosenLabel = this.container.find('.ranges li:last').addClass('Selected').attr('data-range-key');
                } else {
        ...
        keydown: function (e) {
            ...
            if (e.keyCode === 191 || (e.keyCode >= 48 && e.keyCode <= 57))
                this.hide(false);
        },

        FormatInputValue: function (e) {

            var val = this.element.val().replace(new RegExp('[^0-9\/]', 'g'), '');

            if (e.keyCode != 191 && (val === '' || e.keyCode < 48 || e.keyCode > 57)) {
            ...
            switch (sections.length) {
                ...
                case 3:
                    if (sections[1].length == 1)
                        val = sections[0] + '/' + '0' + sections[1] + '/' + sections[2];

                    if (sections[2].length > 4)
                        val = sections[0] + '/' + sections[1] + '/' + sections[2].slice(0, 4);
                    break;

                default:
                    while (val.split('/').length > 3)
        ...
        updateElement: function () {
            ...
            if (this.element.is('input') && this.autoUpdateInput) {
                ...
                if (newValue !== this.element.val() && this.startDate.isValid())
        ...
        GetStartDate: function (allowNull) {
            if (allowNull || this.startDate.isValid())
        ...
        GetEndDate: function (allowNull) {
            if (allowNull || this.endDate.isValid())
                return this.endDate;
